(function ($) {
  ...
  sliders.forEach((slider) => {
    const nextBtn = slider.parentElement.querySelector(
      ".used-product-slider-next"
    );
    const prevBtn = slider.parentElement.querySelector(
      ".used-product-slider-prev"
    );
  ...
  const backgrounds = document.querySelectorAll(
    ".spa-banner-img-group .single-banner-bg"
  );
  ...
  if (buttons.length > 0 && backgrounds.length > 0) {
    ...
    function removeActiveClasses() {
      buttons.forEach((button) =>
        button.querySelector(".banner-sm-img").classList.remove("active")
      );
    ...
    if (diff >= priceGap && maxp <= rangeInputvalue[1].max) {
      if (e.target.className === "min-input") {
        ...
        rangevalue.style.right = `${
          100 - (maxp / rangeInputvalue[1].max) * 100
        }%`;
      }
    }
  });
...
function initializeSlider() {
  const minValue = parseInt(priceInputvalue[0].value);
  const maxValue = parseInt(priceInputvalue[1].value);

  rangeInputvalue[0].value = minValue;
  rangeInputvalue[1].value = maxValue;

  rangevalue.style.left = `${(minValue / rangeInputvalue[0].max) * 100}%`;
  rangevalue.style.right = `${
    100 - (maxValue / rangeInputvalue[1].max) * 100
  }%`;
}
